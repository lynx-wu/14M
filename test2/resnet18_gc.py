...
class CustomDataset(Dataset):
    def __init__(self, csv_file, root_dir, transform=None, split='train', sample_size=10000):
        ...
        # 过滤 idx 值大于 1000000 的数据
        self.data_frame['idx0'] = self.data_frame['idx'].apply(lambda x: int(x.split('.')[0]))
        self.data_frame = self.data_frame[self.data_frame['idx0'] > 1000000]
    ...
    def __getitem__(self, idx):
        ...
        original_label = self.data_frame.iloc[idx, self.data_frame.columns.get_loc('is_gc')]
...
def train_val_data_process():
    ...
    # 加载数据集
    csv_file = '/media/disk/Backup/02drive/17qiulin/SynFundus-1M_gen_cond_hunhe.csv'
    root_dir = '/media/disk/Backup/02drive/SynFundus-1M/SunFundus-1M-unzip/Fundus'
...
def train_val_model_process(model, train_loader, val_loader, num_epochs):
    ...
    # 创建CSV文件以保存指标
    metrics_file = '/media/disk/Backup/02drive/17qiulin/indexs/indexs.csv'
    ...
    for epoch in range(num_epochs):
        ...
        if epoch >= 4:
            ...
            plt.legend()
            plt.savefig(f'/media/disk/Backup/02drive/17qiulin/graphics/_{epoch + 1}_Accuracy.png')
            plt.close()
            ...
            plt.legend()
            plt.savefig(f'/media/disk/Backup/02drive/17qiulin/graphics/_{epoch + 1}_F1_Score.png')
            plt.close()
            ...
            plt.legend()
            plt.savefig(f'/media/disk/Backup/02drive/17qiulin/graphics/_{epoch + 1}_Recall.png')
            plt.close()
            ...
            plt.legend()
            plt.savefig(f'/media/disk/Backup/02drive/17qiulin/graphics/_{epoch + 1}_Precision.png')
            plt.close()
            ...
            plt.legend()
            plt.savefig(f'/media/disk/Backup/02drive/17qiulin/graphics/_{epoch + 1}_ROC_AUC.png')
            plt.close()

        # 保存模型
        model_save_path = f'/media/disk/Backup/02drive/17qiulin/models/_{epoch + 1}.pth'
...
if __name__ == '__main__':
    ...
    model = CustomResNet18(dropout_rate=0.5).to(device)
    print('test')
    train_loader,val_loader = train_val_data_process()
    print('test1')
    train_val_model_process(model, train_loader, val_loader, 20)
    print('test2')